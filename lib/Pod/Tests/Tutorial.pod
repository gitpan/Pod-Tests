...
Pod::Tests::Tutorial - Tutorial docs for Pod::Tests

=head1 DESCRIPTION

Pod::Tests is a way to embed tests in the same file as your source
...
Install Pod::Tests, but you've probably already done that.  Pod::Tests
will install Test::More and Test::Simple.

=item B<2>

Find some code you want to test.  Pod::Tests works best if this code
...
Read the docs for Test::Simple if you haven't already.  You'll be
making use of its C<ok()> function.  Pod::Tests actually uses
Test::More, but to keep things simple we'll just use the Test::Simple
...
Its best to put the test right next to the documentation its testing.
    ...
    ok( @p == 1,                'Found our scurvy dog' );
    ok( $p[0] eq 'Roberts',     '  The Dread Pirate Roberts!' );
...
examples:
    ...
    ok( $_STDERR_ =~ /^Beware the Ides of March!/,  'warn'  );
...
=item If I write a module with embedded tests, does it depend on Pod::Tests?

Oddly enough, no.  You can write a program with embedded tests and
distribute it B<without> requiring Pod::Tests.  Simply generate the .t
